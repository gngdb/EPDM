        ...
        if uq == "mc":
            ...
            for i in xrange(200):
                ...
                for line in data:
                    ...
                    Ve = 1.5
                    Rf = 1.0
                    #ASSUMING HERE THE VALUES ARE INPUT AS STANDARD DEVIATIONS
                    nl.append(random.gauss(Ra, Ra*(line[0]/100)))
                    nl.append(random.gauss(Rb, Rb*(line[1]/100)))
                    nl.append(random.gauss(Rc, Rc*(line[2]/100)))
                    nl.append(random.gauss(Rd, Rd*(line[3]/100)))
                    nl.append(random.gauss(Ve, Ve*(line[4]/100)))
                    nl.append(random.gauss(Rf, Rf*(line[5]/100)))
                    mcdata.append(nl)
                mclist.append(mcdata)
                datalist.append(begin(mcdata))
            f = open("mcdata.pickle", "w")
            pickle.dump(mclist, f)
            f.close()
            ...
            pdata = map(lambda x: x[0]+x[1], zip(data,mcout))
            #pickle results
            #open pickle file
            try:
...
def powercalc(line):
    ...
    Vthy = (Ve*(Rd+Rc))/(Rd+Rc+Rf)
    Rthy = (Rf*(Rd+Rc))/(Rf+Rc+Rd)
    Vy = (Rb*Vthy)/(Rthy+Ry+Rb)
    Pb = (Vy**2)/Rb
    #Vz is the point next to Rc
    Vthz = (Ve*(Ry+Rb))/(Rf+Ry+Rb)
    Rthz = (Rf*(Ry+Rb))/(Rf+Ry+Rb)
    Vz = (Vthz*Rc)/(Rthz+Rd+Rc)
    Pc = (Vz**2)/Rc
    ...
    Pd = ((Vx-Vz)**2)/Rd
    Pf = ((Ve-Vx)**2)/Rf
    #hah, recalculate Rbridge here because I wrote this bit befor the rest
        ...
        if uq == "mc":
            ...
            for i in xrange(200):
                ...
                for line in data2:
                    ...
                    nl=[]
                    for R in line[:4]:
                        #5.9 instead of 6, as that's what the research turned up
